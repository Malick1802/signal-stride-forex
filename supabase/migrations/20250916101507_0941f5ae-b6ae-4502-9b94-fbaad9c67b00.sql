-- Fix ambiguous column reference by renaming local variable in check_signal_outcomes
...
DECLARE
  ...
  all_tps_hit BOOLEAN;
  exit_price NUMERIC;
  v_expire_reason TEXT; -- renamed from expire_reason to avoid ambiguity
BEGIN
    ...
    IF NOT outcome_exists THEN
      ...
      IF NOT stop_loss_hit AND signal_record.take_profits IS NOT NULL AND array_length(signal_record.take_profits, 1) > 0 THEN
        ...
        IF all_tps_hit THEN
          ...
          v_expire_reason := 'all_take_profits_hit';
        END IF;
      END IF;
      ...
      IF stop_loss_hit OR all_tps_hit THEN
        ...
        SET 
          status = 'expired',
          updated_at = NOW(),
          expire_reason = v_expire_reason,
          expired_by = 'database_trigger',
          last_expiration_check = NOW()
        WHERE id = signal_record.id;
        
        -- Log the expiration for debugging
        RAISE NOTICE 'Signal % expired: reason=%, exit_price=%', signal_record.id, v_expire_reason, exit_price;
      END IF;
    END IF;
  END LOOP;
