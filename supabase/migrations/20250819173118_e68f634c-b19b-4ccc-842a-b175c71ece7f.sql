...
BEGIN
  ...
  FROM public.calculate_signal_performance(ts.price, NEW.current_price, ts.type, ts.symbol) AS perf
  WHERE ts.symbol = NEW.symbol 
...
-- Update existing active signals with current performance
UPDATE public.trading_signals ts
SET 
...
FROM public.centralized_market_state cms,
     LATERAL public.calculate_signal_performance(ts.price, cms.current_price, ts.type, ts.symbol) AS perf
