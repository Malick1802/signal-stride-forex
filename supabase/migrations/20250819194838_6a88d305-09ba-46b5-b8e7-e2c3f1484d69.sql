-- Add audit trail columns to trading_signals
ALTER TABLE public.trading_signals 
ADD COLUMN IF NOT EXISTS expire_reason text,
ADD COLUMN IF NOT EXISTS expired_by text DEFAULT 'system',
ADD COLUMN IF NOT EXISTS last_expiration_check timestamp with time zone DEFAULT now();

-- Add processed_by column to signal_outcomes
ALTER TABLE public.signal_outcomes 
ADD COLUMN IF NOT EXISTS processed_by text DEFAULT 'system';

-- Update the check_signal_outcomes function to only expire when ALL TPs are hit OR stop loss is hit
