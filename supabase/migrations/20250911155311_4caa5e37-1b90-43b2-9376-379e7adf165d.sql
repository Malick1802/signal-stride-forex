...
CREATE TABLE IF NOT EXISTS public.optimal_trading_parameters (
  ...
  -- Ensure only one active config per pair/timeframe
  UNIQUE(pair, timeframe, active) WHERE active = TRUE
);
...
CREATE INDEX IF NOT EXISTS idx_optimal_params_active ON public.optimal_trading_parameters(pair, timeframe) WHERE active = TRUE;
