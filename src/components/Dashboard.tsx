      ...
      <div className="md:hidden bg-black/10 backdrop-blur-sm border-b border-white/10">
        <div className="px-4 py-3">
          <Tabs value={activeTab} onValueChange={onTabChange || setInternalActiveTab} className="w-full">
            <TabsList className={`grid w-full ${isAdmin ? 'grid-cols-4' : 'grid-cols-2'} bg-slate-700/80 backdrop-blur-sm border border-slate-600/50 p-1 gap-0 h-12 rounded-xl overflow-hidden`}>
              <TabsTrigger 
                value="signals" 
                className="text-sm font-medium text-slate-300 data-[state=active]:text-white data-[state=active]:bg-emerald-600 data-[state=active]:shadow-none rounded-lg px-3 py-2 h-10 transition-all duration-200"
              >
                Signals
              </TabsTrigger>
              <TabsTrigger 
